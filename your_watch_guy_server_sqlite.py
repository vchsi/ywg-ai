...
# db import
from MSQLDBWriter import YWGDBWriter, MySQLDBWriter
...
db_path = "ywg_db"
dbWriter = YWGDBWriter(database=db_path)
...
def generateReportForId(db_connection, id, id_data, system_instructions):
    ...
    i = 0
    conn = db_connection.getNewConnection()
    try:
        ...
        db_connection.load_report(id, reportData, conn=conn)
        db_connection.change_report_credits(id, conn=conn)
        db_connection.update_report_status(id, False, conn=conn)
        db_connection.update_columns("ywg_tracker_table", id, {"report": True}, conn=conn)
        print("Report for ", id, "done.")
    finally:
        db_connection.releaseConnection(conn)
    return
    
# Thread to generate reports independently from web server operations
class ReportThread(threading.Thread):
    def __init__(self, dbWriter: MySQLDBWriter, id: int, content: dict, system_instructions):
